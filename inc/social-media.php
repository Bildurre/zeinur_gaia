...
/**
 * Get social media networks configuration
 * 
 * @return array Array of social networks with their data
 */
function z_gaia_get_social_networks() {
  return array(
    'facebook' => array(
      'label' => __('Facebook', 'z-gaia'),
 ...
 */
function z_gaia_social_media($args = array()) {
  $defaults = array(
    'show_label' => false,
    'class'      => '',
  );
  ...
  foreach ($networks as $network => $data) {
    $url = get_theme_mod("z_gaia_social_{$network}", '');
    if (!empty($url)) {
      $has_social = true;
      break;
    }
  ...
  foreach ($networks as $network => $data) {
    ...
    if (!empty($url)) {
      echo '<a href="' . esc_url($url) . '" class="social-media__link social-media__link--' . esc_attr($network) . '" target="_blank" rel="noopener noreferrer" aria-label="' . esc_attr($data['label']) . '">';
      echo '<span class="social-media__icon">' . $data['icon'] . '</span>';
      
      if ($args['show_label']) {
        echo '<span class="social-media__label">' . esc_html($data['label']) . '</span>';
      }
